...
const categories = ["All", "Platform", "Event", "Tool", "Infrastructure", "E-Commerce", "Hackathon"];
    ...
    <div className="min-h-screen pt-24 pb-32 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col gap-6">
          {projects.map((p, i) => (
            <motion.article
              ...
              className="group border border-[rgba(149,254,124,0.1)] bg-[#0d1321]/40 hover:border-[rgba(149,254,124,0.25)] hover:bg-[#0d1321]/70 transition-all p-6 md:p-8 card-shine"
            >
              {/* Header row */}
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-6">
                <div>
                  <div className="flex flex-wrap items-center gap-3 mb-2">
                    ...
                    <span className="flex items-center gap-1.5 font-inter text-xs text-[#4a4f62]">
                      <span className={`w-1 h-1 rounded-full ${p.status === "Live · Solana Mainnet" || p.status === "Live" || p.status === "Live · First Sale Confirmed" || p.status === "Live · Production" || p.status === "Live · Running Daily" ? "bg-[#95fe7c] animate-pulse-dot" : "bg-[#4a4f62]"}`} />
                      {p.status}
                    </span>
                  </div>
                  <h2 className="font-rajdhani font-bold text-[#f8f8f0] text-2xl md:text-3xl group-hover:text-[#95fe7c] transition-colors mb-1">
                    {p.label}
                  </h2>
                ...
                <a
                  ...
                  className="self-start font-rajdhani font-bold text-xs uppercase tracking-wider px-4 py-2 border border-[rgba(149,254,124,0.2)] text-[#8a8fa8] hover:border-[#95fe7c] hover:text-[#95fe7c] transition-colors whitespace-nowrap"
                >
                  Visit →
                </a>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 mb-6 pb-6 border-b border-[rgba(149,254,124,0.06)]">
                {p.stats.map((s) => (
                  <div key={s.label}>
                    <span className="font-rajdhani font-bold text-[#95fe7c] text-lg md:text-xl block leading-tight">
                      {s.value}
                    </span>
                ...
                <div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {p.tech.map((t) => (
                        <span key={t} className="text-[10px] font-inter text-[#4a4f62] border border-[rgba(149,254,124,0.08)] px-2 py-0.5">
                          {t}
                        </span>
                      ))}
                    </div>
        ...
        <motion.div
          ...
          className="mt-20 p-8 border border-[rgba(149,254,124,0.1)] bg-[#0d1321]/40"
        >
          <span className="font-rajdhani font-bold text-xs uppercase tracking-widest text-[#95fe7c] block mb-4">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
            {[
              ...
              <div key={t.name} className="p-4 border border-[rgba(149,254,124,0.08)] bg-[#08090f]/60">
